...
function List() {
    ...
    const fetchContractData = async () => {
        let url = `https://quanlyhopdong-be.hisoft.vn/Contracts/partner?IsApproved=false&CurrentPage=1&pageSize=20`;
    ...
    const fetchNext = async () => {
        ...
        const res = await fetch(`https://quanlyhopdong-be.hisoft.vn/Contracts/partner?IsApproved=false&CurrentPage=${currentPage + 1}&pageSize=20`, {
            mode: 'cors',
            method: 'GET',
            headers: {
                Authorization: `Bearer ${token}`,
    ...
    const handleKeyDown = async (e) => {
        if (e.key === 'Enter') {
            let url = `https://quanlyhopdong-be.hisoft.vn/Contracts/partner?CurrentPage=1&PageSize=20&ContractName=${searchByName}&IsApproved=false`;
            const res = await fetch(url, {
                mode: 'cors',
                method: 'GET',
                headers: {
    ...
    const handleSubmit = async (event) => {
        ...
        setIsFilterOpen(false);
        let url = `https://quanlyhopdong-be.hisoft.vn/Contracts/partner?IsApproved=false&CurrentPage=1&PageSize=10&Code=${contractCode}`;
        if (version > 0) {
    ...
    return (<div className='partner-waiting-contract-list'>
                    ...
                    <tbody>
                        {contracts.map(contract => (
                            <tr className="intro-x">
                                ...
                                <td>{contract.effectiveDateString}</td>
                                <td>
