...
function Comment() {
  ...
  const fetchComments = async () => {
    try {
      const response = await fetch(
        `https://quanlyhopdong-be.hisoft.vn/Comments/annex?contractAnnexId=${contractAnnexId}&CurrentPage=1&PageSize=10`,
        {
          mode: "cors",
          method: "GET",
          headers: new Headers({
  ...
  const fetchNext = async () => {
    ...
    const res = await fetch(`https://quanlyhopdong-be.hisoft.vn/Comments/annex?contractAnnexId=${contractAnnexId}&CurrentPage=${currentPage + 1}&pageSize=10`, {
      mode: 'cors',
      method: 'GET',
      headers: {
        Authorization: `Bearer ${token}`,
  ...
  const handleKeyDown = async (e) => {
    if (e.key === 'Enter') {
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ "contractAnnexId": contractAnnexId, "content": content, "replyId": 0, "commentType": 1 })
      });
      if (res.status === 200) {
        setContent('');
        fetchComments();
